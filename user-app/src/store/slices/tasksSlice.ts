...
export const pauseTask = createAsyncThunk(
  ...
  async (taskId: string, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(error.response?.data?.message || 'Failed to pause task');
    }
  }
);
...
export const resumeTask = createAsyncThunk(
  ...
  async (taskId: string, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(error.response?.data?.message || 'Failed to resume task');
    }
  }
);
...
export const stopTask = createAsyncThunk(
  ...
  async (taskId: string, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(error.response?.data?.message || 'Failed to stop task');
    }
  }
);
...
export const updateTask = createAsyncThunk(
  'tasks/update',
  async ({ taskId, data }: { taskId: string; data: Partial<TaskData> }, { rejectWithValue }) => {
    try {
...
export const carryForwardTasks = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(error.response?.data?.message || 'Failed to carry forward tasks');
    }
  }
);
...
const tasksSlice = createSlice({
  ...
  reducers: {
    ...
    updateTaskTimer: (state, action: PayloadAction<{ taskId: string; seconds: number }>) => {
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(startTask.fulfilled, (state, action) => {
      const taskIndex = state.tasks.findIndex(t => t._id === action.payload.task._id);
      if (taskIndex !== -1) {
    ...
    builder.addCase(completeTask.fulfilled, (state, action) => {
      ...
      if (state.stats) {
        ...
        if (action.payload.task.status === 'in_progress') state.stats.inProgress -= 1;
        if (action.payload.task.status === 'pending') state.stats.pending -= 1;
        if (action.payload.task.status === 'paused') state.stats.paused -= 1;
      }
    });
