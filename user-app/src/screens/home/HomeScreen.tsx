...
export default function HomeScreen() {
  ...
  const saveExpoToken = async () => {
    if (expoPushToken && user?._id) {
      try {
        await ExpoTokenAPI.saveToken({ userId: user._id, token: expoPushToken })
      } catch (error) {
  ...
  const { todayAttendance, hasCheckedIn, hasCheckedOut, isLoading } = useAppSelector(
  ...
  const { elapsedTime, totalSeconds } = useWorkTimer(checkInTime, checkOutTime, hasCheckedOut);

  // Calculate progress (9 hour workday = 32400 seconds)
  const standardWorkSeconds = 9 * 60 * 60;
  const workProgress = hasCheckedIn && !hasCheckedOut 
    ...
    try {
      ...
      if (status === 'granted') {
        try {
          ...
          showToast.warning('Location Error', 'Could not get location. Continuing without it.');
        }
      }
  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={['top']}>
      <LoadingOverlay
        ...
        message={hasCheckedIn && !hasCheckedOut ? 'Finishing your work day...' : 'Starting your work day...'}
      />
      <ScrollView
          ...
          >
            ...
            >
              ...
              <View style={styles.swipePage}>
                <TouchableOpacity
                  style={[styles.startButton, (isLoading || isProcessing) && styles.buttonDisabled]}
                  onPress={!hasCheckedIn ? handleCheckIn : hasCheckedOut ? undefined : handleCheckOut}
                ...
                >
                  <LinearGradient
                    colors={hasCheckedIn && !hasCheckedOut 
                      ...
                      : ['#4ADE80', '#A3E635']}
                    start={{ x: 0, y: 0 }}
                    end={hasCheckedIn && !hasCheckedOut 
                      ? { x: 1, y: 1 } 
                      : { x: 1, y: 0 }}
                  ...
                  >
                    {isLoading && !isProcessing ? (
                      ...
                      <>
                        ...
                        {!hasCheckedOut && <View style={styles.finishWorkContainer}>
                  ...
                  <View style={styles.attendanceCard}>
                    ...
                    >
                      <Text style={styles.attendanceCardTitle}>
                        Start working at {todayAttendance.checkInTime || formatTime12Hour(todayAttendance.checkIn.time)}
                      </Text>
                      {todayAttendance.isLate && (
                        <View style={styles.lateWarningContainer}>
                          <Ionicons name="warning" size={18} color="#FFFFFF" />
                          <Text style={styles.lateWarningText}>
                            Arrived at work {todayAttendance.lateByFormatted || formatLateTime(todayAttendance.lateBy)} late
                          </Text>
                        </View>
                      )}
                    </LinearGradient>
