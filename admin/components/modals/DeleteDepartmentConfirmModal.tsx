...
import { deleteDepartment, fetchDepartments } from '../../lib/slices/departmentSlice';
import toast from 'react-hot-toast';

export default function DeleteDepartmentConfirmModal() {
  const dispatch = useAppDispatch();
  const isOpen = useAppSelector((state) => state.ui.modals.deleteDepartmentConfirm);
  const isLoading = useAppSelector((state) => state.departments.isLoading);
  const selectedDepartment = useAppSelector((state) => state.ui.selectedDepartment);

  const handleClose = () => {
    dispatch(closeModal('deleteDepartmentConfirm'));
  };
  ...
  if (!isOpen || !selectedDepartment) return null;
            ...
            <div className="mt-4">
              ...
              {selectedDepartment.description && (
                <p className="mt-2 text-sm text-gray-500">
                  Description: <span className="font-medium text-gray-700">{selectedDepartment.description}</span>
                </p>
              )}
              {selectedDepartment.employeeCount > 0 && (
                <p className="mt-2 text-sm text-red-600">
                  Warning: This department has {selectedDepartment.employeeCount} employee(s). Deleting it may affect their records.
                </p>
              )}
            </div>
          </div>
