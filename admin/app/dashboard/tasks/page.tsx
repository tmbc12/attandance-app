...
export default function TasksPage() {
  ...
  const [statusFilter, setStatusFilter] = useState<'all' | 'pending' | 'in_progress' | 'completed' | 'paused'>('all');
  ...
  const loadTasksData = async () => {
    ...
    try {
      const response = await api.get('/api/tasks/employee-wise', {
        params: { date: selectedDate, status: statusFilter, priority: priorityFilter }
      });
  ...
  const getFilteredEmployeeWiseTasks = () => {
    ...
    console.log(`[info] - getFilteredEmployeeWiseTasks - before filtered`, JSON.stringify(filtered));

    if (statusFilter !== 'all') {
      filtered = filtered.map(t => ({ ...t, tasks: t.tasks.filter(task => task.status === statusFilter) }));
    }

    if (priorityFilter !== 'all') {
      filtered = filtered.map(t => ({ ...t, tasks: t.tasks.filter(task => task.priority === priorityFilter) }));
    }

    if (searchTerm) {
      filtered = filtered.map(t => ({ ...t, tasks: t.tasks.filter(task => {
        const lastHistory = task.history && task.history.length > 0 ? task.history[task.history.length - 1] : null;
        if (!lastHistory) return false;
        return (
          lastHistory.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
          lastHistory.description.toLowerCase().includes(searchTerm.toLowerCase()) ||
          t.employee.name.toLowerCase().includes(searchTerm.toLowerCase())
        );
      }) }));
    }
    ...
    console.log(`[info] - getFilteredEmployeeWiseTasks - after filtered`, JSON.stringify(filtered));
  ...
  const exportToExcel = () => {
    ...
    const headers = ['Employee ID', 'Employee Name', 'Department', 'Task Title', 'Description', 'Priority', 'Status', 'Time Spent', 'Created Date'];
    const rows = filtered.map(task => {
      return [
        ...
        task.history && task.history.length > 0 ? task.history[task.history.length - 1].title : 'No title',
        task.history && task.history.length > 0 ? task.history[task.history.length - 1].description : 'No description',
  ...
  const stats = {
    ...
    completed: employeeWiseTask.reduce((sum, group) => sum + group.tasks.filter(t => t.status === 'completed').length, 0),
    inProgress: employeeWiseTask.reduce((sum, group) => sum + group.tasks.filter(t => t.status === 'in_progress').length, 0),
    pending: employeeWiseTask.reduce((sum, group) => sum + group.tasks.filter(t => t.status === 'pending').length, 0)
  };
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow p-4">
        <div className="flex flex-wrap items-center gap-4">
          ...
          <select
            value={statusFilter}
            onChange={(e) => setStatusFilter(e.target.value as 'all' | 'pending' | 'in_progress' | 'completed' | 'paused')}
        ...
        getFilteredEmployeeWiseTasks().length > 0 ? (
          <div className="columns-1 gap-6 md:columns-3 xl:columns-4 p-4">
            {getFilteredEmployeeWiseTasks().map((task) => (
              <EmployeeCard key={task.employee._id} employee={task.employee} tasks={task.tasks} />
            ))}
          </div>
        ) : (
          <div className="text-center py-12">
            ...
            <p className="text-gray-500 mt-1">No tasks found for the selected date and filters</p>
          </div>
        )}
      </div>
    </div>
