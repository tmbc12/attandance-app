...
router.post('/employee-login', async (req, res) => {
  try {
    const { email, password } = req.body;
    console.log('📱 Employee login attempt:', { email, hasPassword: !!password });
    ...
    const employee = await Employee.findOne({ email, status: 'active' }).populate('department', '_id name');
...
router.post('/send-password-reset-otp', async (req, res) => {
  try {
    ...
    res.status(500).json({
      error: 'ServerError',
      message: 'An error occurred during send password reset OTP. Please try again later.'
    });
  }
});
...
router.post('/verify-password-reset-otp', async (req, res) => {
  try {
    ...
    res.status(500).json({
      error: 'ServerError',
      message: 'An error occurred during verify password reset OTP. Please try again later.'
    });
  }
});
